        ...
        .controller('pickupIndexCtrl', ['$scope', '$ui', '$store', '$data', '$timeout', '$request', '$state',
            function ($scope, $ui, $store, $data, $timeout, $request, $state) {
                ...
                $scope.submit = function (invalid) {
                    ...
                    var data = {};
                    if ($scope.showChoose) {
                        data.userId = $scope.userInfo._id.$id;
                    }
                    ...
                    data.state = 0;
                    $request.post('api/?model=pickup&action=create',
                        data,
                        function (response) {
                            if (response.success) {
                                $state.go('module', {
                                    'module': 'system',
                                    'action': 'success',
                                    'params': 'title=恭喜您，下单成功！&message=稍后会有工作人员联系您取件，请保持手机畅通。'
                                });
