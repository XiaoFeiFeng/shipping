    ...
    angular.module('channelIndexModule', [])
        .controller("channelIndexCtrl", ['$scope', '$ui', '$request', '$validate', function ($scope, $ui, $request, $validate) {
            ...
            $scope.getChannels = function (type) {
                $request.get(
                    'api/?model=channel&action=logistics_category_channels&category=' + type.code,
                    function (response) {
                        if (response.success) {
            ...
            $scope.modifyState = function (channel) {
                ...
                $request.post('api/?model=channel&action=edit_logistics_category_channels&id=' + channel._id.$id,
                    data,
                    function (response) {
                        if (response.success) {
                            $scope.getChannels($scope.currentCategory);
            ...
            $scope.delChannel = function (id) {
                $ui.confirm('确认删除?', '确认',
                    function () {
                        $request.get('api/?model=channel&action=remove_logistics_category_channels&id=' + id,
                            function (response) {
                                if (response.success) {
                                    $ui.notify('删除成功', '提示');
                                    $scope.getChannels($scope.currentCategory);
